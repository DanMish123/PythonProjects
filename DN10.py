...
class Kolesar:
    ...
    def pojdi(self, smer):
        if smer == ">" and not self.je_mrtev:
            if self.x < len(self.zemljevid[0]) - 1:
                self.x += 1
                self.prevozena_razdalja += 1
            else: self.je_mrtev = True
        ...
        if smer == "v" and not self.je_mrtev:
...
class Zbiralec(Kolesar):
    def __init__(self, x, y, zemljevid):
        ...
        self.x = x
        self.y = y
        self.znamenitosti = set()
        self.naj_obiskane = defaultdict(int)

    def pojdi(self, smer):
        super().pojdi(smer)
        znamenitost = self.zemljevid[self.y][self.x]
        if znamenitost != ".":
            self.znamenitosti.add(znamenitost)
            self.naj_obiskane[znamenitost] += 1

    def znacke(self):
        return self.znamenitosti

    def naj_znacke(self):
        naj_obiskane_znacke = set()
        if self.naj_obiskane:
            naj_vrednost = max(self.naj_obiskane.values())
            for znacka, vrednost in self.naj_obiskane.items():
                if vrednost == naj_vrednost:
                    naj_obiskane_znacke.add(znacka)
        return naj_obiskane_znacke

    def trofeje(self):
        naj_pogoste = []
        naj_obiskane_sorted = {}
        if self.naj_obiskane:
            naj_obiskane_sorted = {val[0] : val[1] for val in sorted(self.naj_obiskane.items(), key=lambda x: (-x[1], x[0]))}
            for znamenitost, vrednost in naj_obiskane_sorted.items():
                if len(naj_pogoste) < 3:
                    naj_pogoste.append((znamenitost,vrednost))
                else:
                    if naj_pogoste[-1][1] == vrednost:
                        naj_pogoste.append((znamenitost,vrednost))
                    else: break
            return naj_pogoste
        else: return []
...
class Sledi:
    def __init__(self):
        self.sledi = {}
    def dodaj_sled(self, lokacija, smer):
        self.sledi[lokacija] = smer
...
class Drsalec(Zbiralec):
    ...
    def pojdi(self, smer):
        self.sledi.dodaj_sled(self.lokacija(), smer)
        super().pojdi(smer)
        if self.lokacija() in self.sledi.sledi:
            for _ in range(3):
                super().pojdi(self.sledi.sledi[self.lokacija()])
                if self.lokacija() not in self.sledi.sledi:
                    self.sledi.dodaj_sled(self.lokacija(), smer)
                    break
