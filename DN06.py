def nove_povezave(pot, zemljevid):
    return {(a,b) for a,b in zip(pot, pot[1:]) if (a,b) not in zemljevid}

def obiskane_tocke(pot):
    return {tocka for tocka in pot}

def popravljena_pot(pot):
    return pot[0] + ''.join([b for a,b in zip(pot, pot[1:]) if a != b])

def povezave_z_vescino(pot, zemljevid, vescina):
    return [(povezava[0], povezava[1]) for povezava in zip(pot, pot[1:]) if vescina in zemljevid[povezava]]

def dolgocasna_pot(pot, zemljevid):
    return any(not zemljevid[povezava] for povezava in zip(pot, pot[1:]))

def dobra_pot(pot, zemljevid):
    return all(len(zemljevid[povezava]) >= 2 for povezava in zip(pot, pot[1:]))

def zahtevnost_poti(pot, zemljevid):
    return max(len(zemljevid[povezava]) for povezava in zip(pot, pot[1:]))

def izvedljiva(pot, zemljevid, zivljenj):
    return zivljenj - sum(povezava not in zemljevid for povezava in zip(pot, pot[1:])) > 0

def enosmerne(zemljevid):
    return {(a,b) for (a,b) in zemljevid if (b,a) not in zemljevid}

def dvosmerne(zemljevid):
    return {(a,b): v for (a,b),v in zemljevid.items() if (b,a) in zemljevid}

def najzahtevnejsi_odsek(pot, zemljevid):
    return max([(a, b) for a,b in zip(pot, pot[1:])], key=lambda x: len(zemljevid[x]))
