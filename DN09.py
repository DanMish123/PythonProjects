...
def preberi_zemljevid(ime_dat):
    vrstice = open(ime_dat)
    zemljevid = {}
    for i, vrstica in enumerate(vrstice):
        vrstica = vrstica.strip()
        for j, crka in enumerate(vrstica):
            if crka != ".":
                if crka not in zemljevid:
                    zemljevid[crka] = []
                zemljevid[crka].append((j, i))
    return zemljevid

def najblizji(x, y, c, zemljevid, prepovedani):
    if c:
        najblizja_koordinati = None
        najblizja = sys.maxsize
        for (x1,y1) in zemljevid[c]:
            if (x1, y1) != (x, y) and (x1,y1) not in prepovedani:
                razdalja = distance(x,y,x1,y1)
                if razdalja < najblizja:
                    najblizja = razdalja
                    najblizja_koordinati = (x1,y1)
                elif razdalja == najblizja:
                    if x1 < najblizja_koordinati[0] or (x1 == najblizja_koordinati[0] and y1 < najblizja_koordinati[1]):
                        najblizja_koordinati = (x1, y1)
        return najblizja_koordinati
    else:
        najblizja_koordinati = None
        najblizja = sys.maxsize
        for crka, koordinati in zemljevid.items():
            for (x1, y1) in koordinati:
                if (x1, y1) != (x, y) and (x1,y1) not in prepovedani:
                    razdalja = distance(x, y, x1, y1)
                    if razdalja < najblizja:
                        najblizja = razdalja
...
def najpogostejsi(x, y, d, zemljevid):
    crke = defaultdict(int)
    for crka, koordinati in zemljevid.items():
        for (x1,y1) in koordinati:
            if distance(x,y,x1,y1) <= d:
                crke[crka] += 1
    if crke: return min(crke, key=lambda k: (-crke[k], k))
    else: return None
    ...
    for crka, koordinati in zemljevid.items():
        for (x1, y1) in koordinati:
            if distance(x, y, x1, y1) <= d:
                crke[crka] += 1
    if crke:
        najpogostejsa = max(crke.values())
        return set(crka for crka, koliko in crke.items() if najpogostejsa == koliko)
    else: return set()
...
def angelca(x, y, znamenitosti, zemljevid):
    ...
    for znamenitost in znamenitosti:
        najblizja = najblizji(trenutna[0], trenutna[1], znamenitost, zemljevid, obiskani)
        if najblizja is None: break
        obiskani.add(najblizja)
        suma += distance(trenutna[0], trenutna[1], najblizja[0], najblizja[1])
        trenutna = najblizja
    return suma

def vsi_koordinati(zemljevid):
    return {koord: k for k,v in zemljevid.items() for koord in v}
...
def johanca(x, y, pot, zemljevid):
    ...
    while i < len(pot):
        ...
        while pot[i].isdigit():
            num += pot[i]
            i+=1
        if num: koraki = int(num)
        else: koraki = 1
        znak = pot[i]
        if znak == ">":
            trenutna = (trenutna[0] + koraki, trenutna[1])
        elif znak == "<":
            trenutna = (trenutna[0] - koraki, trenutna[1])
        elif znak == "^":
            trenutna = (trenutna[0], trenutna[1] - koraki)
        elif znak == "v":
            trenutna = (trenutna[0], trenutna[1] + koraki)
        koordinati = vsi_koordinati(zemljevid)
        if trenutna in koordinati and trenutna not in obiskani:
            znamenitosti += koordinati[trenutna]
            obiskani.add(trenutna)
        i+=1
    return znamenitosti

def max_x(zemljevid):
    return max(x for koord in zemljevid.values() for (x,y) in koord)
def max_y(zemljevid):
    return max(y for koord in zemljevid.values() for (x,y) in koord)

def najboljsa_cetrt(a, zemljevid):
    koordinati = (0,0)
    max_znamenitosti = 0
    vsi = vsi_koordinati(zemljevid)
    x1 = max_x(zemljevid)
    y1 = max_y(zemljevid)
    for x in range(x1 - a + 1):
        for y in range(y1):
            znamenitosti = 0
            for i in range(x, x+a):
                for j in range(y, y+a):
                    if (i,j) in vsi:
                        znamenitosti+=1
            if znamenitosti > max_znamenitosti:
                max_znamenitosti = znamenitosti
                koordinati = (x,y)
            elif znamenitosti == max_znamenitosti:
                if x < koordinati[0] or (x == koordinati[0] and y < koordinati[1]):
                    koordinati = (x,y)
    return koordinati
